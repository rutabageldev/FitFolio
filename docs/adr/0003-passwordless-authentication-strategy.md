...
security, authentication, backend, ux

## Context

FitFolio needed an authentication system that prioritizes both security and user
experience. Traditional password-based authentication has well-known problems:
...
**No traditional passwords.** Users authenticate via email (magic link) or
biometrics/security keys (WebAuthn).
