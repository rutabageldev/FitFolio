...
# Deps tests are unit-level (in-memory DB), but security-critical
...
@pytest.mark.asyncio
async def test_get_current_session_cookie_attributes_default_flags(
    db_session, test_user, monkeypatch
):
    """
    Default cookie flags should include HttpOnly and SameSite=Lax.
    Secure should be absent by default.
    """
    # Ensure COOKIE_SECURE is not set (or false)
    ...
    assert "secure" not in set_cookie  # only present when COOKIE_SECURE=true
...
@pytest.mark.asyncio
async def test_get_optional_session_old_session_triggers_rotation(
    db_session, test_user
):
    """Optional dep should also rotate old sessions and set new cookie."""
    token = create_session_token()
    token_hash = hash_token(token)
    old_session = Session(
        user_id=test_user.id,
