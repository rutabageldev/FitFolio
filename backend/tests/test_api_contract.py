...
These tests verify that all documented API endpoints are accessible at their
expected paths. This catches regressions when API versioning or routing changes.
...
@pytest.fixture
async def client():
    """Create test client for API contract tests."""
...
class TestAPIVersioning:
    """Verify API versioning structure is correct."""

    async def test_api_v1_prefix_required(self, client: AsyncClient):
        """All versioned endpoints should require /api/v1/ prefix."""
        # Try old path (should fail)
        ...
        assert response.status_code in (
            403,
            404,
        ), "Old /auth path should not work (403=CSRF, 404=not found)"

    async def test_magic_link_at_versioned_path(self, client: AsyncClient):
        """Magic link should be at /api/v1/auth/magic-link/start."""
        response = await client.post(
            "/api/v1/auth/magic-link/start",
            json={"email": "test@example.com"},
        )
        # May be 200 or rate limited, but should not be 404
        assert response.status_code != 404, "Versioned endpoint should exist"

    async def test_me_endpoint_at_versioned_path(self, client: AsyncClient):
        """GET /me should be at /api/v1/auth/me."""
        response = await client.get("/api/v1/auth/me")
        # Will be 401 (unauthorized) but NOT 404 (not found)
        assert response.status_code == 401, "Should exist but require auth"

    async def test_admin_endpoints_versioned(self, client: AsyncClient):
        """Admin endpoints should be at /api/v1/admin/."""
        response = await client.get("/api/v1/admin/audit/event-types")
        # Will be 401 (unauthorized) but NOT 404 (not found)
        assert response.status_code == 401, "Should exist but require auth"
...
class TestEndpointInventory:
    """
    Comprehensive inventory of all expected endpoints.

    This test will fail if endpoints are moved, renamed, or removed,
    forcing explicit acknowledgment of API contract changes.
    """

    # Expected endpoint paths (method, path, expected_status_without_auth)
    EXPECTED_ENDPOINTS = [
        # Health & Debug (unversioned)
        ("GET", "/healthz", 200),
        ("POST", "/_debug/mail", None),  # Dev only, may not exist in prod
        # Auth endpoints (versioned)
        ("POST", "/api/v1/auth/magic-link/start", 200),  # Public
        ("POST", "/api/v1/auth/magic-link/verify", 400),  # Needs token
        ("POST", "/api/v1/auth/webauthn/register/start", 401),  # Needs auth
        ("POST", "/api/v1/auth/webauthn/register/finish", 401),  # Needs auth
        ("POST", "/api/v1/auth/webauthn/authenticate/start", 400),  # Needs email
        ("POST", "/api/v1/auth/webauthn/authenticate/finish", 400),  # Needs data
        ("GET", "/api/v1/auth/webauthn/credentials", 401),  # Needs auth
        ("POST", "/api/v1/auth/logout", 401),  # Needs auth
        ("GET", "/api/v1/auth/me", 401),  # Needs auth
        # Email verification (versioned)
        ("POST", "/api/v1/auth/email/verify", 400),  # Needs token
        ("POST", "/api/v1/auth/email/resend-verification", 400),  # Needs email
        # Session management (versioned)
        ("GET", "/api/v1/auth/sessions", 401),  # Needs auth
        ("POST", "/api/v1/auth/sessions/revoke-all-others", 401),  # Needs auth
        # Admin (versioned)
        ("GET", "/api/v1/admin/audit/events", 401),  # Needs auth
        ("GET", "/api/v1/admin/audit/event-types", 401),  # Needs auth
        # API Docs (unversioned)
        ("GET", "/docs", 200),
        ("GET", "/redoc", 200),
        ("GET", "/openapi.json", 200),
    ]

    @pytest.mark.parametrize("method,path,expected_status", EXPECTED_ENDPOINTS)
    async def test_endpoint_exists(
        ...
        """
        Verify each endpoint exists at its documented path.

        This test doesn't check functionality (other tests do that),
        just that the endpoint is reachable and returns something
        ...
        assert response.status_code != 404, (
            ...
            f"Endpoint may have been moved or removed. "
            f"If this is intentional, update EXPECTED_ENDPOINTS."
        )

        # For endpoints we know the exact status, verify it
        if expected_status and response.status_code != expected_status:
            # This is a warning, not a failure - status might vary by auth state
            print(
