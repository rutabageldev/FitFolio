...
class TestInactiveUserEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_me_endpoint_inactive_user_rejected(
        self, client: AsyncClient, csrf_token, inactive_user_with_session
    ):
        """Should reject /me request from inactive user."""
        user, session_token = inactive_user_with_session
