...
# Admin tests exercise HTTP endpoints and are integration-level
...
class TestAuditLogEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_invalid_user_id_returns_400(self, client: AsyncClient, db_session):
        """Invalid user_id format should return 400."""
        now = datetime.now(UTC)
        user = User(
            email="badfilter@test.com",
            is_active=True,
            is_email_verified=True,
            created_at=now,
            updated_at=now,
        ...
        resp = await client.get(
            "/api/v1/admin/audit/events?user_id=not-a-uuid",
            cookies={"ff_sess": session_token},
        )
        assert resp.status_code == 400

    @pytest.mark.asyncio
    async def test_date_range_filters(self, client: AsyncClient, db_session):
        """start_date and end_date should bound returned entries."""
        now = datetime.now(UTC)
        user = User(
        ...
        # Create three events at t0, t1, t2 minutes
        for i in range(3):
            db_session.add(
                LoginEvent(
                    user_id=user.id,
        ...
        # Filter to only include events at >= t1 and <= t2
        start = (now + timedelta(minutes=1)).replace(tzinfo=None).isoformat()
        end = (now + timedelta(minutes=2)).replace(tzinfo=None).isoformat()
        resp = await client.get(
            f"/api/v1/admin/audit/events?start_date={start}&end_date={end}",
            cookies={"ff_sess": session_token},
        )
        ...
        created_times = [datetime.fromisoformat(e["created_at"]) for e in entries]
