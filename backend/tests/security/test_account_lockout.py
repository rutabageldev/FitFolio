...
class TestAccountLockoutHelpers:
    ...
    @pytest.mark.asyncio
    async def test_lockout_additional_failures_still_locked(self):
        """After lockout, additional failures keep account locked with remaining TTL."""
        redis_client = await get_redis()
        test_user_id = uuid.uuid4()
        ...
        # Additional failure attempt shouldn't change locked state (won't retrigger)
        should_lock, _ = await record_failed_login(redis_client, test_user_id)
        assert should_lock is False
        is_locked, seconds_remaining = await check_account_lockout(
            redis_client, test_user_id
        )
        assert is_locked and seconds_remaining is not None

    @pytest.mark.asyncio
    async def test_lockout_expires_and_resets(self):
        """Lockout should expire after TTL and allow login attempts again."""
        redis_client = await get_redis()
        test_user_id = uuid.uuid4()
        lockout_key = f"lockout:{test_user_id}"

        # Manually set a very short lockout to avoid long sleeps
        now = datetime.now(UTC)
        lockout_until = now + timedelta(seconds=1)
        await redis_client.set(lockout_key, lockout_until.isoformat(), ex=1)
