...
class TestCSRFMethods:
    ...
    @pytest.mark.asyncio
    async def test_put_nonexistent_requires_csrf(self, client):
        """Even for non-existent paths, CSRF should intercept unsafe methods first."""
        response = await client.put("/nonexistent-path", json={"foo": "bar"})
        assert response.status_code == 403
        assert "CSRF" in response.json().get("detail", "")

    @pytest.mark.asyncio
    async def test_delete_nonexistent_requires_csrf(self, client):
        """DELETE without CSRF should be blocked by middleware before routing."""
        response = await client.delete("/also-nonexistent")
        assert response.status_code == 403
        assert "CSRF" in response.json().get("detail", "")


class TestDocsAndOpenAPI:
    """Docs and OpenAPI should not be CSRF-blocked on safe methods."""

    @pytest.mark.asyncio
    async def test_docs_not_csrf_blocked(self, client):
        resp = await client.get("/docs")
        assert resp.status_code != 403

    @pytest.mark.asyncio
    async def test_redoc_not_csrf_blocked(self, client):
        resp = await client.get("/redoc")
        assert resp.status_code != 403

    @pytest.mark.asyncio
    async def test_openapi_not_csrf_blocked(self, client):
